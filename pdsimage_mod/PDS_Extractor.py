...
class BinaryTable(object):
    ...
    def _detect_size(self, url):
        ...
        meta = site.info()
        return float(meta.get('Content-Length')) / 1e6
    ...
    def _maybe_download(self):
        ...
        if self.grid == 'WAC':
            ...
            images = [str(elt).split('\"')[1].split('.')[0]
                      for elt in r.iter_lines() if ".IMG" in str(elt)]
            if self.fname not in images:
                raise ValueError("%s : Image does not exist\n.\
                                 Possible images are:\n %s" % (self.fname, '\n, '.join(images[2:])))

            elif (not os.path.isfile(self.img)) and (self.fname in images):
                urlname = os.path.join(urlpath, os.path.split(self.img)[-1])
                print("The size is ?: %.1f MB \n\n" %
                      (self._detect_size(urlname)))
                download = True#self._user_yes_no_query('Do you really want to download %s ?\n\n' % (self.fname))
                if download:
                    self._downloadfile(urlname, self.img)
                else:
                    raise ValueError("You need to download the file somehow")

                urlname = os.path.join(urlpath, os.path.split(self.lbl)[-1])
